...
def find_age(age,gender):
    for i in range(0, len(age)):
        if (len(age[i]) < 5): ## 한자리 숫자 범위 일때 기준값으로 변환
            if (int(age[i][0:1]) + 2 >= 1 and int(age[i][0:1]) + 2 <= 5):
                age[i] = "1~5"
            elif (int(age[i][0:1]) + 2 >= 6 and int(age[i][0:1]) + 2 <= 10):
                age[i] = "6~10"
            else:
                age[i] = "11~15"
        if (len(age[i]) >= 5): # 두자리 숫자 범위 일때 기준값으로 변환
            if (int(age[i][0:2]) + 2 >= 11 and int(age[i][0:2]) + 2 <= 15):
                age[i] = "11~15"
            elif (int(age[i][0:2]) + 2 >= 16 and int(age[i][0:2]) + 2 <= 20):
                age[i] = "16~20"
            elif (int(age[i][0:2]) + 2 >= 21 and int(age[i][0:2]) + 2 <= 25):
                age[i] = "21~25"
            elif (int(age[i][0:2]) + 2 >= 26 and int(age[i][0:2]) + 2 <= 30):
                age[i] = "26~30"
            elif (int(age[i][0:2]) + 2 >= 31 and int(age[i][0:2]) + 2 <= 35):
                age[i] = "31~35"
            elif (int(age[i][0:2]) + 2 >= 36 and int(age[i][0:2]) + 2 <= 40):
                age[i] = "36~40"
            elif (int(age[i][0:2]) + 2 >= 41 and int(age[i][0:2]) + 2 <= 50):
                age[i] = "41~50"
            elif (int(age[i][0:2]) + 2 >= 51 and int(age[i][0:2]) + 2 <= 60):
                age[i] = "51~60"
            else:
                age[i] = "61~80"
...
if __name__ == "__main__":
    ...
    while True:
       ...
       # cv2.setWindowProperty("window", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)  # 카메라 창 전체화면
        ...
        if count % 100 == 0:  #프레임 100번당 1번 opencv에서 얼굴 인식
            count=0
            if len(faces) != 0:  #open cv에서 사람 인식 한후 밑에 있는 네이버 오픈API 실행
                ...
                headers = {'X-Naver-Client-Id': client_id, 'X-Naver-Client-Secret': client_secret} #naver api id, pwd를 헤더에 저장
                ...
                if result['info']['faceCount'] == 0: # 네이버 API에서 사람 인식 못 할때
                    ...
                    for faceCount in range(len(result['faces'])):
                        print("gender : " + result['faces'][faceCount]['gender']['value'])  # 인식된 gender 출력
                        print("age : " + result['faces'][faceCount]['age']['value'])  # 인식된 age 출력
                        age.append(result['faces'][faceCount]['age']['value']) #성별list에 성별 입력
                        gender.append(result['faces'][faceCount]['gender']['value']) #나이list에 나이 입력
